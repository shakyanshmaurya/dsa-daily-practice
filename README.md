# DSA-daily-practice
Daily DSA practice and problem solving journey.
# 🚀 50 Days DSA Challenge (C Language)

## 📌 About
This repository contains my daily DSA practice using C language.  
I am solving problems consistently to improve my logic building and coding skills.

---

## 📅 Progress

- Day 1 - Basic Array
- Day 2 - Divisible by 3
- Day 3 - Second Largest Element
- Day 4 - Remove Duplicates
- Day 5 - Bubble Sort
- Day 6 - Selection Sort
- Day 7 - Insertion Sort
- Day 8 - Binary Search
- Day 9 - Recursion (Factorial)
- Day 10 - Fibonacci (Recursion)
- Day 11 - Fibonacci (Loop)
- Day 12 - 2D Array
- Day 13 - Matrix Addition
- Day 14 - Matrix Transpose
- Day 15 - Pointers Basics
- Day 16 - Swap using Pointer
- Day 17 - Array using Pointer
- Day 18 - Sum using Pointer
- Day 19 - Largest Element
- Day 20 - Smallest Element
- Day 21 - Reverse Array
- Day 22 - Linear Search
- Day 23 - Binary Search
- Day 24 - Bubble Sort
- Day 25 - Selection Sort / Insert Element
- Day 26 - Delete Element
- Day 27 - Merge Arrays
- Day 28 - Second Largest
- Day 29 - Prime Number
- Day 30 - Palindrome Number
- Day 31 - String Length
- Day 32 - Reverse String
- Day 33 - Palindrome String
- Day 34 - Vowels & Consonants
- Day 35 - Remove Spaces / Count Words
- Day 36 - Duplicate Characters
- Day 37 - Remove Duplicates String
- - Day 37 - Remove Duplicate Characters (String)
- Day 38 - Anagram Check
- Day 39 - First Non-Repeating Character
- Day 40 - String Rotation Check
- Day 41 - Sum of Digits (Recursion)
- Day 42 - Power of Number (Recursion)
- Day 43 - Linked List Create & Traverse
- Day 44 - Insert Node at Beginning (Linked List)
- Day 45 - Insert Node at End (Linked List)
  Day 46 — Array Traversal

Day 47 — Linear Search

Day 48 — Find Maximum Element

Day 49 — Find Minimum Element

Day 50 — Array Sum

Day 51 — Reverse Array

Day 52 — Second Largest Element

Day 53 — Check Sorted Array

Day 54 — Remove Duplicates

Day 55 — Move Zeroes

Day 56 — Left Rotate Array

Day 57 — Right Rotate Array

Day 58 — Rotate Array by K

Day 59 — Union of Arrays

Day 60 — Intersection of Arrays

Day 61 — Majority Element

Day 62 — Kadane’s Algorithm

Day 63 — Buy & Sell Stock

Day 64 — Merge Sorted Arrays

Day 65 — Missing Number

Day 66 — Longest Subarray Sum K

Day 67 — Sort 0s, 1s, 2s

Day 68 — Find Duplicate Number

Day 69 — Merge Intervals

Day 70 — Next Greater Element

Day 71 — Valid Parentheses

Day 72 — Binary Search

Day 73 — Recursive Binary Search



---

🔹 Sorting Algorithms

Day 74 — Bubble Sort

Day 75 — Selection Sort

Day 76 — Insertion Sort

Day 77 — Merge Sort

Day 78 — Quick Sort

Day 79 — Count Inversions

Day 80 — Heap Sort



---

🔹 Heap Data Structure

Day 81 — Min Heap

Day 82 — Max Heap

Day 83 — Priority Queue Basics

Day 84 — Heap Operations

Day 85 — Heapify Process

Day 86 — Heap Applications

Day 87 — Advanced Heap Concepts

Day 88 — Heap Practice Problems

Day 89 — Heap Sort Revision

Day 90 — Min Heap Implementation

Day 91 — Max Heap Implementation 

---

## 🎯 Goal
- Improve problem-solving skills
- Build consistency
- Strengthen DSA concepts

---

## 💻 Language Used
- C Programming

---

## ⭐ Note
This is my learning journey. I am improving day by day 🚀
